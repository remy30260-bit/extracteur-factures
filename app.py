...
# ══════════════════════════════════════════════════════════════════════════════
# CSS GLOBAL
# CORRECTIF : tout le CSS est dans un seul bloc st.markdown, sans code Python dedans
# ══════════════════════════════════════════════════════════════════════════════
st.markdown("""
<style>
...
.topbar {
    ...
    padding: 0 2rem; gap: 2rem; z-index: 1001;
...
.topbar-nav a:hover, .topbar-nav a.active {
    background: #f7f5f3; color: #1a1a2e; font-weight: 600;
}
...
.chat-header-title { font-size: 0.9rem; font-weight: 700; color: white; }
.chat-header-sub { font-size: 0.72rem; color: rgba(255,255,255,0.8); }
.chat-messages {
    flex: 1; overflow-y: auto; padding: 1rem;
    display: flex; flex-direction: column; gap: 0.75rem;
}
.msg-bot { display: flex; gap: 0.5rem; align-items: flex-start; }
.msg-user { display: flex; justify-content: flex-end; }
.bubble {
    padding: 0.5rem 0.75rem; border-radius: 12px;
    font-size: 0.82rem; max-width: 85%; line-height: 1.4;
}
.msg-bot .bubble { background: #f7f5f3; color: #374151; }
.msg-user .bubble { background: linear-gradient(135deg, #f0a070, #e07040); color: white; }
...
def analyse_facture_gemini(images: list, api_key: str) -> dict:
    """Extraction des données de facture via Gemini Vision."""
...
Réponds UNIQUEMENT avec le JSON, sans markdown ni texte autour.
{
  ...
  "tva": 0.00,
  "montant_ttc": 0.00,
  "categorie": "Transport|Repas|Hébergement|Matériel|Télécom|Formation|Autre",
  "statut": "À payer"
}
Si une information est absente, utilise null pour les nombres et "" pour les textes."""

    parts = [prompt] + images
    try:
        response = model.generate_content(parts)
        text = response.text.strip()
        # Nettoyer si Gemini entoure quand même de ```json
        if text.startswith("```"):
            text = text.split("```")[1]
...
defaults = {
    ...
    # CORRECTIF : clé unifiée "gemini_api_key" partout
    "gemini_api_key":        "",
}
...
# ══════════════════════════════════════════════════════════════════════════════
# TOPBAR + NAVIGATION
# CORRECTIF : navigation uniquement via st.session_state + st.rerun()
#             La sidebar reste masquée. Les boutons nav sont dans la topbar.
# ══════════════════════════════════════════════════════════════════════════════
...
nav_html = "".join(
    f'<a class="{"active" if page == k else ""}">{label}</a>'
...
# Boutons Streamlit invisibles superposés sur la topbar
st.markdown("""
<style>
/* Bande cliquable positionnée sur la topbar */
div[data-testid="stHorizontalBlock"]:has(button[kind="secondary"]) {
    position: fixed !important;
    top: 0 !important; left: 180px !important;
    height: 58px !important; z-index: 1002 !important;
...
div[data-testid="stHorizontalBlock"]:has(button[kind="secondary"]) .stButton > button {
    ...
    color: transparent !important; height: 58px !important;
    cursor: pointer !important;
}
div[data-testid="stHorizontalBlock"]:has(button[kind="secondary"]) .stButton > button:hover {
    background: rgba(247,245,243,0.5) !important;
}
</style>
""", unsafe_allow_html=True)
...
st.markdown("""
<style>
/* Bouton FAB fixé en bas à droite */
div[data-testid="stVerticalBlock"]:has(> div > div > button[kind="secondary"]#chat_fab_btn) {
    position: fixed !important;
    bottom: 2rem !important; right: 2rem !important;
    z-index: 2000 !important; width: 56px !important;
}
</style>
""", unsafe_allow_html=True)
...
if st.session_state["chat_open"]:
    ...
    if send and user_msg.strip():
        ...
        if any(w in msg_lower for w in ["facture", "invoice"]):
            response = "📄 Pour vos factures, allez dans **Factures** et utilisez l'import IA 🐾"
            ...
            response = "📊 La Comptabilité vous donne une vue consolidée des charges et TVA 🐾"
        elif any(w in msg_lower for w in ["export", "csv", "json"]):
            response = "📥 L'export CSV et JSON est disponible dans chaque section 🐱"
        elif any(w in msg_lower for w in ["bonjour", "salut", "hello", "coucou"]):
            response = "Bonjour ! 🐱 Je suis FactureCat. Que puis-je faire pour vous ?"
            ...
            response = "🔑 Configurez votre clé Gemini dans **Paramètres > Clé API** !"
        else:
            response = "🐱 Essayez de me parler de vos factures, notes de frais ou de la comptabilité !"
...
if page == "dashboard":
    ...
    with col_d2:
        st.markdown("""
        <div class="panel">
            <div class="panel-header"><span class="panel-title">🏷️ Statuts</span></div>
        ...
        if statuts_count:
            for s, count in statuts_count.items():
                ...
                <div style="display:flex;justify-content:space-between;
                     align-items:center;padding:0.4rem 0;border-bottom:1px solid #f7f5f3;">
                    {badge(s)}
        ...
        <div class="panel" style="margin-top:1rem;">
            <div class="panel-header"><span class="panel-title">⚡ Accès rapide</span></div>
            <div class="panel-body">
        """, unsafe_allow_html=True)
        if st.button("📤 Importer des factures", use_container_width=True, key="dash_import"):
            st.session_state["page"] = "factures"; st.rerun()
        if st.button("💸 Nouvelle note de frais", use_container_width=True, key="dash_ndf"):
            st.session_state["page"] = "notes_frais"; st.rerun()
        if st.button("📊 Voir la comptabilité", use_container_width=True, key="dash_compta"):
            st.session_state["page"] = "comptabilite"; st.rerun()
        st.markdown('</div></div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        <div style="display:flex;align-items:center;gap:1rem;margin-bottom:1.5rem;
             background:white;border-radius:14px;padding:1rem 1.5rem;border:1px solid #ece9e4;">
            ...
            <div>
                ...
                <span style="color:#6b7280;font-size:0.82rem;">
                    ...
                    toutes les informations grâce à Gemini 🐾
                </span>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if uploaded_files:
            ...
            if new_files:
                ...
                with col_sel:
                    ...
                    selected_preview = st.selectbox("👁️ Aperçu rapide", names, key="select_preview")
                    for f in new_files:
                        if f.name == selected_preview:
                            f.seek(0); content = f.read(); f.seek(0)
                            imgs = pdf_to_images(content) if f.name.lower().endswith(".pdf")                                    else [Image.open(io.BytesIO(content))]
                ...
                with col_prev:
                    if st.session_state["current_preview"]:
                        for img in st.session_state["current_preview"][:2]:
                            # CORRECTIF : use_container_width remplace use_column_width (déprécié)
                            st.image(img, use_container_width=True)

                st.markdown("<div style='height:1rem'></div>", unsafe_allow_html=True)
                b1, b2, b3 = st.columns([1, 2, 1])
                with b2:
                    analyse_btn = st.button("🐱 Analyser avec Gemini IA",
                                            use_container_width=True,
                                            type="primary", key="btn_analyse")

                if analyse_btn:
                    # CORRECTIF : clé unifiée "gemini_api_key"
                    ...
                    if not api_key:
                        ...
                        with st.status("🐱 Analyse en cours…", expanded=True) as status:
                            for f in new_files:
                                ...
                                f.seek(0); content = f.read(); f.seek(0)
                                images = pdf_to_images(content) if f.name.lower().endswith(".pdf")                                          else [Image.open(io.BytesIO(content))]
                        ...
                        for fname, result, _ in results:
                            with st.expander(f"📄 {fname}", expanded=True):
                                ...
                                for col_r, fields in [(c1, fields_c1),(c2, fields_c2),(c3, fields_c3)]:
                                    with col_r:
                                        for lbl, val in fields:
                                            st.markdown(f"""
                                            <div style="margin-bottom:0.5rem;">
                                                <div style="font-size:0.7rem;color:#9ca3af;font-weight:600;">{lbl}</div>
                                                <div style="font-size:0.9rem;font-weight:700;color:#1a1a2e;">{val}</div>
                                            </div>
                                            """, unsafe_allow_html=True)
            else:
                st.markdown("""
                <div class="upload-zone">
                    ...
                    <p style="font-weight:700;color:#1a1a2e;font-size:1rem;margin:0;">Aucun fichier sélectionné</p>
                    <p style="color:#9ca3af;font-size:0.84rem;margin:0.5rem 0 0 0;">Utilisez le bouton ci-dessus</p>
                </div>
                """, unsafe_allow_html=True)
        else:
            st.markdown("""
            <div class="upload-zone">
                <div style="font-size:3rem;margin-bottom:1rem;">🐱</div>
                <p style="font-weight:700;color:#1a1a2e;font-size:1rem;margin:0;">Glissez vos factures ici</p>
                <p style="color:#9ca3af;font-size:0.84rem;margin:0.5rem 0 0 0;">PDF, PNG, JPG, JPEG acceptés</p>
            </div>
            """, unsafe_allow_html=True)
    ...
    with tab2:
        ...
        if factures:
            ...
            with f1:
                search = st.text_input("🔍 Rechercher", placeholder="Fournisseur, numéro…", key="fac_search")
            with f2:
            ...
            with col_list:
                ...
                for f in filtered:
                    rows += f"""
                    <tr>
                        <td><b style="color:#1a1a2e;">{f.get('fournisseur','—')}</b><br>
                            <span style="color:#9ca3af;font-size:0.72rem;">{f.get('numero','—')}</span></td>
                ...
                <div class="panel">
                    <table class="invoice-table">
                        <thead><tr><th>Fournisseur</th><th>Date</th><th>TTC</th><th>Statut</th></tr></thead>
                        <tbody>{rows}</tbody>
                    </table>
                </div>
                """, unsafe_allow_html=True)
                ...
                if filtered:
                    sel_names = [
                        f"{f.get('fournisseur','—')} · {f.get('date','—')} · {float(f.get('montant_ttc',0)):,.2f} €"
                        for f in filtered
                    ]
                    sel = st.selectbox("Sélectionner une facture",
                ...
                if idx is not None and idx < len(filtered):
                    ...
                    for lbl, val in [
                        ...
                        <div style="display:flex;justify-content:space-between;
                            ...
                            <span style="font-size:0.8rem;color:#9ca3af;font-weight:500;">{lbl}</span>
                            <span style="font-size:0.84rem;color:#1a1a2e;font-weight:600;">{val}</span>
                        </div>
                        """, unsafe_allow_html=True)
                    st.markdown('</div></div>', unsafe_allow_html=True)
                    ...
                    with a1:
                        new_statut = st.selectbox("Changer statut",
                            ["À payer","Payée","En retard","Annulée"], key=f"statut_{idx}")
                    with a2:
                        if st.button("💾 Sauvegarder", key=f"save_{idx}",
                                     use_container_width=True, type="primary"):
                            try:
                                get_supabase().table("factures").update(
                                    {"statut": new_statut}).eq("id", r["id"]).execute()
                    ...
                    with a3:
                        if st.button("🗑️ Supprimer", key=f"del_{idx}", use_container_width=True):
                            try:
                                ...
                                st.session_state["selected_facture_idx"] = None; st.rerun()
                            except Exception:
                                st.error("❌ Erreur")
                else:
                    st.markdown("""
                    <div class="panel" style="text-align:center;padding:4rem 2rem;">
                        ...
                        <p style="color:#6b7280;font-weight:600;margin-top:0.5rem;">Sélectionnez une facture</p>
                    </div>
                    """, unsafe_allow_html=True)
        else:
            st.markdown("""
        ...
        if factures:
            ...
            with e2:
                st.download_button("📥 Télécharger JSON",
                    data=json.dumps(factures, ensure_ascii=False, indent=2),
                    file_name="factures.json", mime="application/json", use_container_width=True)
        else:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        with col_form:
            ...
            c1, c2 = st.columns(2)
            with c1: ndf_date = st.date_input("📅 Date", value=date.today(), key="ndf_date")
            with c2: ndf_cat  = st.selectbox("📂 Catégorie", list(CAT_ICONS.keys()), key="ndf_cat")

            ndf_desc = st.text_input("📝 Description", key="ndf_desc",
                                     placeholder="Ex: Déjeuner client Paris")
            c3, c4 = st.columns(2)
            with c3: ndf_ht = st.number_input("💵 Montant HT (€)", min_value=0.0, step=0.01, key="ndf_ht")
            with c4: ndf_tva_pct = st.selectbox("📊 TVA", ["20%", "10%", "5.5%", "0%"], key="ndf_tva")
            ...
            <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;
                ...
                <div>
                    ...
                    <div style="font-size:1rem;font-weight:700;color:#065f46;">{ndf_tva_montant:.2f} €</div>
                </div>
                <div>
                    <div style="font-size:0.7rem;color:#6b7280;font-weight:600;">TOTAL TTC</div>
                    <div style="font-size:1rem;font-weight:700;color:#065f46;">{ndf_ttc:.2f} €</div>
                </div>
            </div>
            """, unsafe_allow_html=True)

            c5, c6 = st.columns(2)
            with c5: ndf_paiement = st.selectbox("💳 Moyen de paiement",
                ["Carte bancaire", "Espèces", "Virement", "Chèque"], key="ndf_paiement")
            with c6: ndf_statut = st.selectbox("🏷️ Statut",
                ["En attente", "Validée", "Remboursée", "Refusée"], key="ndf_statut")

            ndf_justificatif = st.file_uploader("📎 Justificatif",
                type=["pdf", "png", "jpg", "jpeg"], key="ndf_justificatif")
            ...
            with b2:
                save_ndf = st.button("💾 Enregistrer la dépense",
                                     use_container_width=True, type="primary", key="btn_save_ndf")
            ...
            if save_ndf:
                if not ndf_desc.strip():
                    ...
                    ndf_data = {
                        "date": str(ndf_date), "categorie": ndf_cat,
                        "description": ndf_desc, "montant_ht": ndf_ht,
                        "tva": ndf_tva_montant, "montant_ttc": ndf_ttc,
                        "moyen_paiement": ndf_paiement, "statut": ndf_statut,
                        "justificatif": ndf_justificatif.name if ndf_justificatif else "",
                    }
                    try:
                        uid = st.session_state.get("user_id", "")
                        get_supabase().table("notes_frais").insert({**ndf_data, "user_id": uid}).execute()
                        st.success("✅ Dépense enregistrée !"); st.rerun()
                    except Exception:
                        st.session_state.setdefault("notes_frais", []).append(ndf_data)
                        st.success("✅ Dépense enregistrée localement !"); st.rerun()

        with col_side:
            ndfs = get_data("notes_frais")
            mois = datetime.now().month; annee = datetime.now().year
            ndfs_mois  = [n for n in ndfs if str(n.get("date","")).startswith(f"{annee}-{mois:02d}")]
            total_mois = sum(float(n.get("montant_ttc", 0)) for n in ndfs_mois)
            en_attente = sum(float(n.get("montant_ttc", 0)) for n in ndfs_mois if n.get("statut") == "En attente")

            st.markdown(f"""
            <div class="panel">
                <div class="panel-header"><span class="panel-title">📅 Ce mois-ci</span></div>
                <div class="panel-body">
                    <div style="display:flex;justify-content:space-between;padding:0.6rem 0;border-bottom:1px solid #f7f5f3;">
                        <span style="font-size:0.8rem;color:#9ca3af;">Dépenses totales</span>
                        <span style="font-weight:700;color:#1a1a2e;">{total_mois:,.2f} €</span>
                    </div>
                    <div style="display:flex;justify-content:space-between;padding:0.6rem 0;border-bottom:1px solid #f7f5f3;">
                        <span style="font-size:0.8rem;color:#9ca3af;">En attente</span>
                        <span style="font-weight:700;color:#f0a070;">{en_attente:,.2f} €</span>
                    </div>
            ...
            if ndfs_mois:
                ...
                st.markdown("""
                <div class="panel">
                    <div class="panel-header"><span class="panel-title">📂 Par catégorie</span></div>
                    <div class="panel-body">
                """, unsafe_allow_html=True)
                for cat, total in sorted(cats_total.items(), key=lambda x: x[1], reverse=True):
                    pct  = (total / total_mois * 100) if total_mois > 0 else 0
                    icon = CAT_ICONS.get(cat, "📦")
                    st.markdown(f"""
                    <div style="margin-bottom:0.75rem;">
                        <div style="display:flex;justify-content:space-between;margin-bottom:0.25rem;">
                            <span style="font-size:0.8rem;color:#4b5563;">{icon} {cat}</span>
                            <span style="font-size:0.8rem;font-weight:600;color:#1a1a2e;">{total:,.2f} €</span>
                        </div>
    ...
    with tab2:
        ...
        if ndfs:
            f1, f2, f3 = st.columns(3)
            with f1: search_ndf = st.text_input("🔍 Rechercher", placeholder="Description…", key="ndf_search")
            with f2:
            ...
            filtered_ndf = [
                n for n in ndfs
                if (not search_ndf or search_ndf.lower() in str(n.get("description","")).lower())
            ...
            with col_nl:
                ...
                for n in filtered_ndf:
                    ...
                    rows_ndf += f"""
                    <tr>
                        <td><b style="color:#1a1a2e;">{icon} {n.get('description','—')}</b><br>
                            <span style="color:#9ca3af;font-size:0.72rem;">{n.get('categorie','—')}</span></td>
                ...
                <div class="panel">
                    <table class="invoice-table">
                        <thead><tr><th>Description</th><th>Date</th><th>TTC</th><th>Statut</th></tr></thead>
                ...
                if filtered_ndf:
                    sel_ndf_names = [
                        f"{CAT_ICONS.get(n.get('categorie','Autre'),'📦')} {n.get('description','—')} · {n.get('date','—')} · {float(n.get('montant_ttc',0)):,.2f} €"
                        for n in filtered_ndf
                    ]
            ...
            with col_nv:
                ...
                if sel_ndf_idx is not None and sel_ndf_idx < len(filtered_ndf):
                    ...
                    <div class="panel">
                        <div class="panel-header">
                            <span class="panel-title">{icon} {n.get('description','—')}</span>
                            {badge(n.get('statut','—'))}
                        </div>
                        <div class="panel-body">
                    """, unsafe_allow_html=True)
                    ...
                    with na1:
                        new_ndf_statut = st.selectbox("Changer statut",
                        ...
                        if st.button("💾 Sauvegarder", key=f"ndf_save_{sel_ndf_idx}",
                            ...
                            try:
                                get_supabase().table("notes_frais").update(
                                    {"statut": new_ndf_statut}).eq("id", n["id"]).execute()
                                st.success("✅ Mis à jour"); st.rerun()
                            except Exception:
                                st.error("❌ Erreur mise à jour")
                    with na3:
                        if st.button("🗑️ Supprimer", key=f"ndf_del_{sel_ndf_idx}", use_container_width=True):
                            try:
                                get_supabase().table("notes_frais").delete().eq("id", n["id"]).execute()
                                st.session_state["selected_ndf_idx"] = None; st.rerun()
                            except Exception:
                                local = st.session_state.get("notes_frais", [])
                                st.session_state["notes_frais"] = [
                                    x for x in local if x.get("description") != n.get("description")]
                                st.session_state["selected_ndf_idx"] = None; st.rerun()
                else:
                    st.markdown("""
                    <div class="panel" style="text-align:center;padding:4rem 2rem;">
                        <div style="font-size:3rem;">🐱</div>
                        <p style="color:#6b7280;font-weight:600;margin-top:0.5rem;">Sélectionnez une dépense</p>
                    </div>
                    """, unsafe_allow_html=True)
        else:
            st.markdown("""
            <div style="text-align:center;padding:4rem;background:white;
                ...
                <p style="color:#9ca3af;">Ajoutez votre première dépense dans l'onglet Nouvelle dépense</p>
            </div>
            """, unsafe_allow_html=True)
    ...
    with tab3:
        ...
        if ndfs:
            ...
            with e1:
                st.download_button("📥 Télécharger CSV",
                    data=df_ndf.to_csv(index=False, encoding="utf-8-sig"),
                    file_name="notes_frais.csv", mime="text/csv", use_container_width=True)
            with e2:
                st.download_button("📥 Télécharger JSON",
                    data=json.dumps(ndfs, ensure_ascii=False, indent=2),
                    file_name="notes_frais.json", mime="application/json", use_container_width=True)
        else:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_c1:
        ...
        with col_g1:
            ...
            if cats_fac:
                df_cats = pd.DataFrame(list(cats_fac.items()), columns=["Catégorie","Montant HT"]).sort_values("Montant HT", ascending=False)
                st.markdown("""<div class="panel"><div class="panel-header"><span class="panel-title">📂 Charges par catégorie</span></div><div class="panel-body">""", unsafe_allow_html=True)
        ...
        with col_g2:
            ...
            for f in factures:
                d = str(f.get("date", ""))
                if len(d) >= 7:
                    monthly[d[:7]] = monthly.get(d[:7], 0) + float(f.get("montant_ttc", 0))
            if monthly:
                df_monthly = pd.DataFrame(sorted(monthly.items()), columns=["Mois","TTC"])
                st.markdown("""<div class="panel"><div class="panel-header"><span class="panel-title">📅 Évolution mensuelle TTC</span></div><div class="panel-body">""", unsafe_allow_html=True)
        ...
        with col_g3:
            ...
            for f in factures:
                ...
                statuts_count[s] = statuts_count.get(s, 0) + 1
            if statuts_count:
                df_statuts = pd.DataFrame(list(statuts_count.items()), columns=["Statut","Nombre"])
                st.markdown("""<div class="panel"><div class="panel-header"><span class="panel-title">🏷️ Répartition par statut</span></div><div class="panel-body">""", unsafe_allow_html=True)
                st.bar_chart(df_statuts.set_index("Statut"), use_container_width=True)
                st.markdown('</div></div>', unsafe_allow_html=True)

        with col_g4:
            if total_fac_ttc > 0 or total_ndf_ttc > 0:
                df_rep = pd.DataFrame({"Type":["Factures","Notes de frais"],"Montant":[total_fac_ttc,total_ndf_ttc]})
                st.markdown("""<div class="panel"><div class="panel-header"><span class="panel-title">⚖️ Factures vs Notes de frais</span></div><div class="panel-body">""", unsafe_allow_html=True)
    ...
    with tab_c2:
        all_ops = []
        for f in factures:
            all_ops.append({"Date": f.get("date","—"), "Type": "Facture",
                "Libellé": f.get("fournisseur","—"), "Catégorie": f.get("categorie","—"),
                "HT": float(f.get("montant_ht",0)), "TVA": float(f.get("tva",0)),
                "TTC": float(f.get("montant_ttc",0)), "Statut": f.get("statut","—")})
        for n in ndfs:
            all_ops.append({"Date": n.get("date","—"), "Type": "Note de frais",
                "Libellé": n.get("description","—"), "Catégorie": n.get("categorie","—"),
                "HT": float(n.get("montant_ht",0)), "TVA": float(n.get("tva",0)),
                "TTC": float(n.get("montant_ttc",0)), "Statut": n.get("statut","—")})

        if all_ops:
            all_ops.sort(key=lambda x: str(x.get("Date","")), reverse=True)
            rows_gl = ""
            for op in all_ops:
                type_badge = ('<span class="badge badge-green">Facture</span>'
                              if op["Type"] == "Facture"
                              else '<span class="badge badge-orange">Note de frais</span>')
                rows_gl += f"""
                <tr>
                    <td>{op['Date']}</td><td>{type_badge}</td>
            ...
            <div class="panel">
                <div class="panel-header">
                    ...
                    <span style="font-size:0.8rem;color:#9ca3af;">{len(all_ops)} opérations</span>
                </div>
                <table class="invoice-table">
                    <thead><tr>
                        <th>Date</th><th>Type</th><th>Libellé</th><th>Catégorie</th>
                        <th>HT</th><th>TVA</th><th>TTC</th><th>Statut</th>
                    </tr></thead>
                    <tbody>{rows_gl}</tbody>
                </table>
            </div>
            ...
            with e1:
                st.download_button("📥 Export CSV Grand livre",
                    data=df_gl.to_csv(index=False, encoding="utf-8-sig"),
                    file_name="grand_livre.csv", mime="text/csv", use_container_width=True)
            with e2:
                st.download_button("📥 Export JSON Grand livre",
                    data=json.dumps(all_ops, ensure_ascii=False, indent=2),
                    file_name="grand_livre.json", mime="application/json", use_container_width=True)
        else:
            st.markdown("""
            <div style="text-align:center;padding:4rem;background:white;border-radius:14px;border:1px solid #ece9e4;">
                <div style="font-size:3rem;">📭</div>
    ...
    with tab_c3:
        tva_par_taux = {}
        for entry in factures + ndfs:
            ht  = float(entry.get("montant_ht",  0))
            tva = float(entry.get("tva",          0))
            ttc = float(entry.get("montant_ttc",  0))
            if ht > 0 and tva > 0:
        ...
        if tva_par_taux:
            ...
            st.markdown(f"""
            <div class="panel">
                <div class="panel-header"><span class="panel-title">🧾 Déclaration TVA par taux</span></div>
                <table class="invoice-table">
                    <thead><tr><th>Taux TVA</th><th>Base HT</th><th>TVA déductible</th><th>Total TTC</th></tr></thead>
            ...
            <div style="background:#fff8f3;border:1px solid #fed7aa;border-radius:12px;
                 padding:1.2rem 1.5rem;margin-top:1rem;display:flex;gap:3rem;align-items:center;">
                <div>
                    <div style="font-size:0.72rem;color:#9ca3af;font-weight:600;text-transform:uppercase;">TVA TOTALE À RÉCUPÉRER</div>
                    <div style="font-size:2rem;font-weight:800;color:#f0a070;">{total_tva_all:,.2f} €</div>
                </div>
                <div style="width:1px;background:#fed7aa;height:50px;"></div>
                <div>
                    <div style="font-size:0.72rem;color:#9ca3af;font-weight:600;text-transform:uppercase;">BASE HT TOTALE</div>
                    <div style="font-size:2rem;font-weight:800;color:#1a1a2e;">{total_ht_all:,.2f} €</div>
                </div>
                <div style="width:1px;background:#fed7aa;height:50px;"></div>
                <div>
                    <div style="font-size:0.72rem;color:#9ca3af;font-weight:600;text-transform:uppercase;">TAUX EFFECTIF MOYEN</div>
                    <div style="font-size:2rem;font-weight:800;color:#1a1a2e;">{(total_tva_all/total_ht_all*100) if total_ht_all > 0 else 0:.1f}%</div>
                </div>
            </div>
            """, unsafe_allow_html=True)
        else:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_p1:
        st.markdown('<div class="form-panel">', unsafe_allow_html=True)
        st.markdown('<div class="form-section-title">Configuration Gemini API</div>', unsafe_allow_html=True)
        st.markdown("""
        <div style="background:#fff8f3;border:1px solid #fed7aa;border-radius:10px;
            ...
            🔑 Obtenez votre clé sur <b>https://aistudio.google.com/app/apikey</b>
        </div>
        """, unsafe_allow_html=True)

        # CORRECTIF : lecture et écriture via "gemini_api_key"
        current_key = st.session_state.get("gemini_api_key", "")
        new_key = st.text_input("🔑 Clé API Gemini", value=current_key,
                                type="password", placeholder="AIza...", key="api_key_input")

        p1, p2, p3 = st.columns([1, 2, 1])
        with p2:
            if st.button("💾 Sauvegarder la clé API", type="primary", use_container_width=True):
                if new_key.strip():
    ...
    with tab_p2:
        st.markdown('<div class="form-panel">', unsafe_allow_html=True)
        st.markdown('<div class="form-section-title">Informations entreprise</div>', unsafe_allow_html=True)
        p1, p2 = st.columns(2)
        with p1:
            st.text_input("🏢 Raison sociale", key="company_name", placeholder="Ma Société SAS")
            st.text_input("🔢 SIRET", key="company_siret", placeholder="XXX XXX XXX XXXXX")
            st.text_input("📍 Adresse", key="company_adresse", placeholder="1 rue de la Paix, 75001 Paris")
        with p2:
            st.text_input("💼 N° TVA intracommunautaire", key="company_tva", placeholder="FR XX XXXXXXXXX")
            st.text_input("📧 Email comptabilité", key="company_email", placeholder="compta@masociete.fr")
            st.text_input("☎️ Téléphone", key="company_tel", placeholder="+33 1 XX XX XX XX")
        pp1, pp2, pp3 = st.columns([1, 2, 1])
        with pp2:
            if st.button("💾 Sauvegarder les informations", type="primary",
                         use_container_width=True, key="save_company"):
                st.success("✅ Informations sauvegardées !")
        st.markdown('</div>', unsafe_allow_html=True)

    with tab_p3:
        st.markdown('<div class="form-panel">', unsafe_allow_html=True)
        st.markdown('<div class="form-section-title">Gestion des données</div>', unsafe_allow_html=True)
        factures = get_data("factures"); ndfs = get_data("notes_frais")
        st.markdown(f"""
        <div style="display:flex;gap:2rem;margin-bottom:1rem;">
            <div style="text-align:center;flex:1;background:#f7f5f3;border-radius:10px;padding:1rem;">
                <div style="font-size:2rem;font-weight:800;color:#1a1a2e;">{len(factures)}</div>
                <div style="font-size:0.8rem;color:#9ca3af;">Factures</div>
            </div>
            <div style="text-align:center;flex:1;background:#f7f5f3;border-radius:10px;padding:1rem;">
                <div style="font-size:2rem;font-weight:800;color:#1a1a2e;">{len(ndfs)}</div>
                <div style="font-size:0.8rem;color:#9ca3af;">Notes de frais</div>
            </div>
            <div style="text-align:center;flex:1;background:#f7f5f3;border-radius:10px;padding:1rem;">
                <div style="font-size:2rem;font-weight:800;color:#f0a070;">{len(factures)+len(ndfs)}</div>
        ...
        d1, d2 = st.columns(2)
        with d1:
            if st.button("🗑️ Vider les factures locales", use_container_width=True):
                st.session_state["factures"] = []; st.success("✅ Supprimées"); st.rerun()
        with d2:
            if st.button("🗑️ Vider les notes de frais locales", use_container_width=True):
                st.session_state["notes_frais"] = []; st.success("✅ Supprimées"); st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
