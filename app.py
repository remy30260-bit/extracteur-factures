...
def render_chat_widget(notes_frais_data=None):
    ...
    function getReply(msg){{
        ...
        // Aide
        if(m.includes('aide') || m.includes('help') || m.includes('comment')){{
            return "🐱 Je peux t'aider avec :<br>• <b>total</b> de tes dépenses<br>• répartition par <b>catégorie</b><br>• la <b>plus grosse</b> dépense<br>• <b>statut</b> des remboursements";
        }}
        
        // Statut
        if(m.includes('statut') || m.includes('rembours')){{
            if(notesData.length === 0) return "Aucune dépense enregistrée 🐱";
            const attente  = notesData.filter(n => n.statut === 'En attente').length;
            const valide   = notesData.filter(n => n.statut === 'Validé').length;
            const rembours = notesData.filter(n => n.statut === 'Remboursé').length;
            return `📊 Statuts :<br>• En attente : ${{attente}}<br>• Validé : ${{valide}}<br>• Remboursé : ${{rembours}} ✅`;
        }}
...
.upload-zone {
    border: 2px dashed rgba(240,160,112,0.5);
    border-radius: 16px;
    padding: 2rem;
    text-align: center;
...
.file-card {
    ...
    padding: 0.6rem 1rem;
    margin: 0.4rem 0;
    border: 1px solid rgba(240,160,112,0.25);
    display: flex;
    align-items: center;
    gap: 0.5rem;
    font-size: 0.82rem;
...
.metric-card {
    ...
    box-shadow: 0 2px 12px rgba(240,112,64,0.07);
    text-align: center;
}
.metric-value {
    font-size: 1.3rem;
...
.stButton > button {
    ...
    box-shadow: 0 3px 10px rgba(240,112,64,0.25) !important;
}
.stButton > button:hover {
    transform: translateY(-1px) !important;
    box-shadow: 0 5px 15px rgba(240,112,64,0.35) !important;
}
...
.dash-card {
    ...
    padding: 1.5rem;
    border: 1px solid rgba(240,160,112,0.2);
    box-shadow: 0 4px 20px rgba(240,112,64,0.08);
    text-align: center;
    margin: 0.3rem 0;
}
...
if st.session_state["page"] == "Factures":
    ...
    with col_left:
        ...
        if uploaded:
            ...
            if new_files:
                st.session_state["extraction_done"] = False  # reset flag si nouveaux fichiers

        # Bubble fichiers en attente
        if st.session_state["pending_files"]:
            st.markdown(f"""
            <div class="cat-container">
                <div class="cat-avatar">🐱</div>
                <div class="cat-bubble">
                    <div class="cat-bubble-title">
                        {len(st.session_state["pending_files"])} fichier(s) prêt(s) ! 🐾
                    </div>
                    <div class="cat-bubble-sub">Clique sur Lancer l'extraction !</div>
                </div>
            </div>
            """, unsafe_allow_html=True)
            ...
            for fname in st.session_state["pending_files"]:
                ...
                st.markdown(f'<div class="file-card">{icon} {fname}</div>',
                            unsafe_allow_html=True)
        ...
        elif st.session_state["factures"]:
            st.markdown('<div class="section-title">📁 Fichiers traités</div>',
                        unsafe_allow_html=True)
            for f in st.session_state["factures"]:
                ext  = f["filename"].split(".")[-1].upper()
                icon = "📄" if ext == "PDF" else "🖼️"
                st.markdown(f'<div class="file-card">{icon} {f["filename"]}</div>',
                            unsafe_allow_html=True)
        ...
        if st.session_state["factures"]:
            ...
            df_dash = pd.DataFrame(st.session_state["factures"])
            total   = df_dash["montant_ttc"].sum() if "montant_ttc" in df_dash.columns else 0
            nb      = len(df_dash)
            ...
            with d1:
                ...
                <div class="dash-card">
                    ...
                    <div class="dash-card-value">{nb}</div>
                    <div class="dash-card-label">Factures</div>
                </div>""", unsafe_allow_html=True)
            with d2:
                st.markdown(f"""
                <div class="dash-card">
                    <div class="dash-card-icon">💰</div>
                    <div class="dash-card-value">{total:.0f}€</div>
                    <div class="dash-card-label">Total TTC</div>
                </div>""", unsafe_allow_html=True)
            st.markdown(f"""
            <div class="dash-card">
                <div class="dash-card-icon">📈</div>
                <div class="dash-card-value">{moy:.0f}€</div>
                <div class="dash-card-label">Moyenne / facture</div>
            </div>""", unsafe_allow_html=True)

    # ── COLONNE CENTRE ──────────────────
    with col_center:

        # Bubble résultat extraction
        ...
        if st.session_state["factures"]:
            ...
            # Prévisualisation
            st.markdown("---")
            st.markdown('<div class="section-title">🔍 Prévisualisation</div>',
                        unsafe_allow_html=True)
            fnames = [f["filename"] for f in st.session_state["factures"]]
            sel    = st.selectbox("Choisir une facture",fnames,
                                  label_visibility="collapsed", key="sel_preview")
            if sel:
                st.session_state["selected_preview"] = sel
                data = st.session_state["uploaded_files_data"].get(sel)
                if data:
                    if data["type"] == "application/pdf":
                        img = pdf_to_image(data["bytes"])
            ...
            <div style="text-align:center;padding:5rem 0;">
                ...
                <p style="color:#c8956c;">Importez vos factures à gauche 🐾</p>
            </div>
            """, unsafe_allow_html=True)
    ...
    with col_metrics:
        ...
        # Total global
        df_all  = pd.DataFrame(st.session_state["factures"]) if st.session_state["factures"] else pd.DataFrame()
        ttc_tot = df_all["montant_ttc"].sum() if "montant_ttc" in df_all.columns else 0
        st.markdown(f"""
        <div style="text-align:center;padding:0.8rem;
                    background:white;border-radius:14px;
                    border:2px solid rgba(200,149,108,0.25);">
            <div style="font-size:0.7rem;color:#c8956c;font-weight:800;">Total toutes factures</div>
            <div style="font-size:1.5rem;font-weight:900;color:#a0522d;">{ttc_tot:.2f} €</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if st.session_state["factures"]:
            ...
            if st.button("🗑️ Effacer tout", use_container_width=True, key="clear_fac"):
    ...
    with col_form:
        ...
        if st.session_state["notes_frais"]:
            ...
            with s1:
                ...
                <div class="dash-card">
                    <div class="dash-card-icon">🧾</div>
                    <div class="dash-card-value">{nb_n}</div>
                    <div class="dash-card-label">Entrées</div>
                </div>""", unsafe_allow_html=True)
            with s2:
                st.markdown(f"""
                <div class="dash-card">
                    <div class="dash-card-icon">💰</div>
                    <div class="dash-card-value">{total_n:.0f}€</div>
